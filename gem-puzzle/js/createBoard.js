...
function createBoard(size = 3, sizeSq = 100, imgSrc, savedGame) {
  ...
  if (typeof savedGame === 'object') {
    ...
    for (let i = 0; i < wrappersElem.length; i += 1) {
  ...
  const finish = (isMakeFinish) => {
    if (!isMakeFinish) {
      ...
      leadersUp.sort(
        (a, b) => a.firstElementChild.textContent.trim() - b.firstElementChild.textContent.trim()
      );
    ...
    for (let i = 0; i < game.children.length; i += 1) {
      game.children[i].children[0].querySelector('.square__num').remove();
    }
  ...
  const startTime = function showTime() {
    if (startGame) {
  ...
  const mapGo = (e) => {
    ...
    let isMove = false;
    for (let arr = 0; arr < mapMove.length; arr += 1) {
      for (let elem = 0; elem < mapMove[arr].length; elem += 1) {
    ...
    if (isMove) {
      ...
      } else if (elemPos.arrNothing < elemPos.arr) {
